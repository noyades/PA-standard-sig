%% This simulation encapsulates data analysis for WiFi 802.11ax (Marketed as
%  Wifi 6, this encapsulates the HE standard)
%
...
%
%  Note on symbol counts: HE uses a 12.8 us FFT, so an OFDM symbol is 16 us
...
sigPath = '..\..\Signals\Multi Carrier\WiFi\802.11AX (WiFi6)\';
figPath = '..\..\Figures\WiFi\802.11AX (WiFi6)\';
...
MAX_HE_SYMBOLS = 339; % 5.484 ms TXOP limit at the default 3.2 us guard interval
...
if runLong || runAll
    ...
    for ibw = 1:numel(bw_list)
        for imcs = 1:numel(mcs_list)
            ...
            cfgHE = papr_std_config('he', bw, mcs, numTX);

            % Solve for the payload that gives the requested symbol count
            % instead of deriving it from an Nbpscs/rate table: HE padding
    ...
    dynamic_filename = fullfile(figPath, sprintf('wifi6_%s_papr_pdf.png', mcs_str));
...
if runStats || runAll
    ...
    targetSymbols = min(env_num('PAPR_STATS_SYMBOLS', 250), MAX_HE_SYMBOLS);
    ...
    cfgHE = papr_std_config('he', BW, MCS, numTX);
    [psduTotalOctets, nSym] = papr_octets_for_symbols(cfgHE, targetSymbols, osf);
    cfgHE = papr_payload(cfgHE, MCS, psduTotalOctets);
    ...
    for c = 1:numChunks
        ...
        txChunk = wlanWaveformGenerator(papr_bits_arg(cfgHE, psduBitsChunk), cfgHE, ...
    ...
    statName = @(kind) fullfile(figPath, ...
        sprintf('wifi6_%s_pdf_mcs=%d_bw=%s.png', kind, MCS, chanBW));
...
if runCdf || runAll
    ...
    % Capped at MAX_HE_SYMBOLS: 1000 symbols would be 16 ms of HE data, three
    ...
    for ib = 1:numel(targetSymbols)
        cfgHE = papr_std_config('he', BW, MCS, numTX);
        [octets, nSym] = papr_octets_for_symbols(cfgHE, ...
            min(targetSymbols(ib), MAX_HE_SYMBOLS), statsOSF);
        cfgHE = papr_payload(cfgHE, MCS, octets);
        achieved(ib) = nSym;
    ...
    fname = fullfile(figPath, sprintf('wifi6_PAPRPDF_%s_mcs=%d_bw=%d.png', modeTag, MCS, BW));
    ...
    fname = fullfile(figPath, sprintf('wifi6_PAPRCCDF_%s_mcs=%d_bw=%d.png', modeTag, MCS, BW));
    fig2 = plot_generic(S.binCenters, S.ccdf, ...
        ...
        'FontSize', 8, 'NColors', 64, 'Save', true);
    fprintf('runCdf done: wrote wifi6_PAPRPDF_%s_mcs=%d_bw=%d.png and wifi6_PAPRCCDF_%s_mcs=%d_bw=%d.png\n', ...
...
if runGen || runAll
    ...
    [measureDataFieldOnly, modeTag] = papr_measure_mode(true);
    [target_mean_papr_db, target_std_papr_db] = papr_target('he', mcs_value, BW, modeTag);
    fprintf('Target for HE MCS %d @ %d MHz (%s): mean=%.3f dB, std=%.3f dB\n', ...
    ...
    genPaprMeta = papr_field_meta(cfgHE, osf, idleTime);
    matched = false;
    for attempt = 1:max_attempts
        psduBits = randi([0 1], 8*octets*numPackets, 1);
        tx_burst = wlanWaveformGenerator(papr_bits_arg(cfgHE, psduBits), cfgHE, ...
